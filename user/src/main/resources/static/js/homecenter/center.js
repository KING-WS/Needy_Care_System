...
// =========================================================
// [NEW] 실시간 노약자 위치 추적 (WebSocket 전용)
// =========================================================
function updateRecipientMarker(lat, lng) {
    ...
    if (!recipientLocationMarker) {
        ...
        if (typeof recipientPhotoUrl !== 'undefined' && recipientPhotoUrl && recipientPhotoUrl !== 'null') {
            ...
            createCircularMarkerImageForRealtime(recipientPhotoUrl, function(dataUrl) {
                if (dataUrl) {
                    ...
                    // 이미지 로드 실패 시 기본 마커
                    var defaultMarkerImage = getDefaultRecipientMarkerImage();
                    createRecipientMarkerOnMap(moveLatLon, defaultMarkerImage);
                }
            });
        ...
        // 이미 마커가 있으면 위치만 부드럽게 이동
        recipientLocationMarker.setPosition(moveLatLon);
        console.log('📍 노약자 위치 업데이트:', lat.toFixed(6), lng.toFixed(6));
    }
}
...
function createCircularMarkerImageForRealtime(photoUrl, callback) {
    ...
    img.onerror = function() {
        console.warn('⚠️ 프로필 이미지 로드 실패:', photoUrl);
        callback(null);
    };
...
function createRecipientMarkerOnMap(position, markerImage) {
    ...
    // 인포윈도우 추가
    var iwContent = '<div style="padding:10px; text-align:center; min-width:150px;">' +
    ...
    console.log("✅ 노약자 마커 생성 완료 (WebSocket)");
}
// ESC 키로 모달 닫기
