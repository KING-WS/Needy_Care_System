...
| 김우성 | 프론트엔드, 백엔드 개발, Admin 모듈 총괄  | rladntjd1193@gmail.com |
...
| 구분 | 기술 | 버전 | 설명 |
| :--- | :--- | :---: | :--- |
| **언어/프레임워크** | **Java** | 17 | 프로젝트 핵심 로직 담당 주력 언어 |
| | **Spring Boot** | 3.5.7 | 애플리케이션 전체 구조 담당 백엔드 프레임워크 |
| **인증/보안** | **Spring Security** | - | 사용자 로그인/로그아웃 및 페이지 접근 권한 관리 |
| | **Jasypt** | 3.0.3 | API 키, DB 비밀번호 등 민감 설정 정보 암호화 |
| **실시간 통신** | **Spring WebSocket** | - | CCTV 위험 감지 알림, 실시간 위치 추적 등 양방향 통신 구현 |
| **AI** | **Spring AI** | 1.0.3 | OpenAI 모델 기반 챗봇, 장소 추천 등 핵심 AI 기능 구현 |

### 🗃️ 데이터베이스 (Database)

| 구분 | 기술 | 버전 | 설명 |
| :--- | :--- | :---: | :--- |
| **데이터베이스** | **MySQL** | - | 프로젝트 주 데이터베이스 (mysql-connector-j) |
| **ORM/Mapper** | **MyBatis** | 3.0.3 | SQL 쿼리 분리(XML) 관리 및 DB 상호작용 처리 |

### 💻 프론트엔드 (Frontend)

| 구분 | 기술 | 버전 | 설명 |
| :--- | :--- | :---: | :--- |
| **뷰 템플릿** | **JSP & JSTL** | 3.0 | 서버 데이터의 동적 HTML 렌더링 |
| **UI 프레임워크** | **Bootstrap** | 5.3.2 | 반응형 웹 디자인 및 UI 컴포넌트(버튼, 모달 등) 활용 |
| **JavaScript** | **jQuery** | 3.7.1 | DOM 조작 및 이벤트 처리 보조 |
| **실시간 통신** | **SockJS & Stomp.js** | 1.6.1 / 2.3.3 | 클라이언트 WebSocket 통신 안정화 라이브러리 |
| **애니메이션** | **AOS** | 2.3.4 | 스크롤 시 콘텐츠 애니메이션 효과 구현 |
| **아이콘** | **Font Awesome** | 6.4.0 | 웹 페이지 전반의 다양한 아이콘 제공 |
| | **Bootstrap Icons** | 1.11.0 | Bootstrap 호환 아이콘 라이브러리 |

### 🔗 주요 API 및 기타 라이브러리

| 구분 | 기술 | 설명 |
| :--- | :--- | :--- |
| **외부 API** | **Kakao Maps API** | 지도 표시, 장소 검색, 주소 변환, 경로 시각화 등 핵심 기능 |
| | **Web Speech API** | 브라우저 내장 API 활용 STT(음성 인식) 및 TTS(음성 합성) |
| **유틸리티** | **Lombok** | 반복 코드(@Getter, @Setter 등) 감소 및 생산성 향상 |
